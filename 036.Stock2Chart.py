...
gnGetBaeDangStockCount = int(gnMaxBaeDangStockCount + (gnMaxBaeDangStockCount * 0.5) + 1);
if (gnGetBaeDangStockCount <= 50):
    gnGetBaeDangStockCount = 50;
gnMaxStockCount = gnGetBaeDangStockCount / 2;
...
def COMPANY_SetStockInfor(stStockInfor, tables, nType, nName, nCode):
    ...
    stStockInfor['시세'] = {};
    bRet = SISE_GetStockInfor(stStockInfor, stStockInfor['시세']);
...
def EXCEL_SetCandleTradeData():
    ...
    nCandelLength = len(gastChartPriceInfor);

    # 엑셀 Sheet 생성
    global gstCandleSheetName;
    gstCandleSheetName = u'캔들매매' + gstDate;
    gstCandleSheet     = gstWorkBook.add_worksheet(gstCandleSheetName);
    gstCandleSheet.freeze_panes('B2');

    stKospiFormat = gstWorkBook.add_format({'font_color': 'purple'});
    stKosdaqFormat = gstWorkBook.add_format({'font_color': 'orange'});
    stBlueFormat = gstWorkBook.add_format({'bold': True, 'font_color': 'blue'});
    stIndicatorFormat = gstWorkBook.add_format({'bold': True, 'font_color': 'brown'});
    stRedTitleFormat = gstWorkBook.add_format({'bold': True, 'font_color': 'red'});
    stGreenTitleFormat = gstWorkBook.add_format({'bold': True, 'font_color': 'green'});
    stPurpleFormat = gstWorkBook.add_format({'bold': True, 'font_color': 'purple'});
    stGrayFormat = gstWorkBook.add_format({'bold': True, 'font_color': 'gray'});
    ...
    nColOffset = nColOffset + 1;
    stString = u'종목명';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stPurpleFormat);
    nColOffset = nColOffset + 1;
    stString = u'날짜';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stPurpleFormat);
    nColOffset = nColOffset + 1;
    stString = u'시가';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stNavyFormat);
    nColOffset = nColOffset + 1;
    stString = u'종가';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stNavyFormat);
    nColOffset = nColOffset + 1;
    stString = u'거래량';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stGrayFormat);
    nColOffset = nColOffset + 1;
    stString = u'평균량';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stGrayFormat);
    nColOffset = nColOffset + 1;
    stString = u'매수날짜';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stRedTitleFormat);
    nColOffset = nColOffset + 1;
    stString = u'매수가';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stRedTitleFormat);
    nColOffset = nColOffset + 1;
    stString = u'매도날짜';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stBlueFormat);
    nColOffset = nColOffset + 1;
    stString = u'매도가';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stBlueFormat);
    nColOffset = nColOffset + 1;
    stString = u'기간';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stGreenTitleFormat);
    nColOffset = nColOffset + 1;
    stString = u'수익여부';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stPurpleFormat);
    nColOffset = nColOffset + 1;
    stString = u'수익률';
    gstCandleSheet.write(nTitleRowOffset, nColOffset, stString, stIndicatorFormat);
    nColOffset = nColOffset + 1;
    nColOffset = nColOffset + 1;
    ...
    stAutoFilterStartCell  = 'A1';
    stAutoFilterEndCell    = 'N1';
    gstCandleSheet.autofilter(stAutoFilterStartCell + ':' + stAutoFilterEndCell);

    if (bPrintLog > 0):
        PrintProgress(u"222");

    # 날짜별 정렬
    for nSrcIndex in range(nCandelLength):
        for nDstIndex in range(nSrcIndex, nCandelLength):
            if (gastChartPriceInfor[nSrcIndex]['날짜'] < gastChartPriceInfor[nDstIndex]['날짜']):
                stTmpChartPriceInfor = gastChartPriceInfor[nSrcIndex];
                gastChartPriceInfor[nSrcIndex] = gastChartPriceInfor[nDstIndex];
                gastChartPriceInfor[nDstIndex] = stTmpChartPriceInfor;

    # 데이터 출력
    nRowOffset = nDataRowOffset;
    for nIndex in range(nCandelLength):
        if (bPrintLog > 0):
            PrintProgress(u"333");

        nColOffset = 0;
        gstCandleSheet.write(nRowOffset, nColOffset, nIndex + 1); # 인덱스
        nColOffset = nColOffset + 1;
        if (gastChartPriceInfor[nIndex]['Type'] == 'KOSPI'):
            gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['종목명'], stKospiFormat);
        else:
            gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['종목명'], stKosdaqFormat);
        nColOffset = nColOffset + 1;
        gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['날짜']);
        nColOffset = nColOffset + 1;
        gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['시가']);
        nColOffset = nColOffset + 1;
        gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['종가']);
        nColOffset = nColOffset + 1;
        gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['거래량']);
        nColOffset = nColOffset + 1;
        gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['평균량']);
        nColOffset = nColOffset + 1;
        gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['매수날짜']);
        nColOffset = nColOffset + 1;
        gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['매수가']);
        nColOffset = nColOffset + 1;
        gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['매도날짜']);
        nColOffset = nColOffset + 1;
        gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['매도가']);
        nColOffset = nColOffset + 1;
        gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['기간']);
        nColOffset = nColOffset + 1;
        if (gastChartPriceInfor[nIndex]['수익률'] >= 0):
            gstCandleSheet.write(nRowOffset, nColOffset, 1, stPlusRateFormat);
        else:
            gstCandleSheet.write(nRowOffset, nColOffset, 0, stMinusRateFormat);
        nColOffset = nColOffset + 1;
        if (gastChartPriceInfor[nIndex]['수익률'] >= 0):
            gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['수익률'], stPlusFormat);
        else:
            gstCandleSheet.write(nRowOffset, nColOffset, gastChartPriceInfor[nIndex]['수익률'], stMinusFormat);
        nBenefitColOffset = nColOffset;
        nColOffset = nColOffset + 1;
        nColOffset = nColOffset + 1;
        ...
        gstCandleSheet.write(nRowOffset, nColOffset, stString, stRateFormat);
        gstCandleSheet.write(nRowOffset + 1, nColOffset, 1, stRateFormat);
        nTransColOffset = nColOffset;
        nColOffset = nColOffset + 1;
...
def CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nIndex):
#    PrintProgress(astKospiInfor[nIndex]['Date']);
    if ((nIndex < len(astKospiInfor)) and (astSiseStockInfor.has_key(astKospiInfor[nIndex]['Date']))):
        stSiseStockInfor = astSiseStockInfor[astKospiInfor[nIndex]['Date']];
    else:
        stSiseStockInfor = 0;

    return stSiseStockInfor;

def CHART_SetTotalTradeCount(astKospiInfor, astSiseStockInfor):
    nBaseCandidateIndex = 60;

    nKospiLength = len(astKospiInfor);

    stSiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, 0);
    nOldTotalTradeCount = 0;
    if (stSiseStockInfor != 0):
        nOldTotalTradeCount = stSiseStockInfor['TradeCount'];

    for nSiseIndex in range(nKospiLength):
        stSiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nSiseIndex);
        if (stSiseStockInfor == 0):
            continue;

        stSiseStockInfor['TotalTradeCount'] = stSiseStockInfor['TradeCount'] + nOldTotalTradeCount;
        nOldTotalTradeCount = stSiseStockInfor['TotalTradeCount'];

        if (nSiseIndex >= nBaseCandidateIndex):
            stSiseStockInfor['ChartCandidate'] = 1;

    nSiseIndex = 1;

def CHART_CheckCandidate(stStockInfor, astKospiInfor, astSiseStockInfor, nIndex, astChartPriceInfor):
    nBaseBuyStartEndRate = 7;
    nBaseBuyTradingIndex = 60;
    nBaseBuyGrowRate = 5;
    nBaseBuyCandleDate = 5;
    nBaseBuyStartTradeDate = 5;
    nBaseBuyTradeDate = 5;
    nBaseBuyFastGrowDate = 10;
    nTargetSellRate = 10;
    nDefenseSellRate = 10;
    nBaseSellTradeDate = 10;

    bPrintLog = 0;

    stSiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nIndex);
    if (stSiseStockInfor == 0):
        return 0;

    if (stSiseStockInfor['ChartCandidate'] == 0):
        return 0;

    if (bPrintLog > 0):
        PrintProgress(u"000   " + astKospiInfor[nIndex]['Date'] + u" " + str(stSiseStockInfor['StartPrice']) + u" "  + str(stSiseStockInfor['EndPrice']));

    # 1. 양봉 7% 상승
    if (stSiseStockInfor['StartEndRate'] < nBaseBuyStartEndRate):
        return 0;

    if (bPrintLog > 0):
        PrintProgress(u"111   " + astKospiInfor[nIndex]['Date'] + u" " + str(stSiseStockInfor['StartPrice']) + u" "  + str(stSiseStockInfor['EndPrice']));

    # 2. 거래량이 이전 60일 평균보다 7배 증가할 경우 선택
    stBaseSiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nIndex - nBaseBuyTradingIndex);
    if (stBaseSiseStockInfor == 0):
        stBaseSiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nIndex - nBaseBuyTradingIndex + 1);
        nBaseBuyTradingIndex = nBaseBuyTradingIndex - 1;
        if (stBaseSiseStockInfor == 0):
            stBaseSiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nIndex - nBaseBuyTradingIndex + 2);
            nBaseBuyTradingIndex = nBaseBuyTradingIndex - 2;

            # 상장된지 얼마되지 않아서 이전 60일이 존재하지 않음
            if (stBaseSiseStockInfor == 0):
                return 0;
    ...
    nAverageTradeCount = (stSiseStockInfor['TotalTradeCount'] - stBaseSiseStockInfor['TotalTradeCount']) / nBaseBuyTradingIndex;
    if (stSiseStockInfor['TradeCount'] <= nAverageTradeCount * nBaseBuyGrowRate):
        return 0;

    if (bPrintLog > 0):
        PrintProgress(u"333");

    # 3. 이후 5일동안 음봉 거래량이 적어야 함
    nPlusCandleTradeCount = 0;
    nMinusCandleTradeCount = 0;
    for nCandleIndex in range(nBaseBuyCandleDate):
        stCandelSiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nIndex + nCandleIndex + 1);
        if (stCandelSiseStockInfor == 0):
            continue;

        if (stCandelSiseStockInfor['StartEndRate'] > 0):
            nPlusCandleTradeCount = nPlusCandleTradeCount + stCandelSiseStockInfor['TradeCount'];
        else:
            nMinusCandleTradeCount = nMinusCandleTradeCount + stCandelSiseStockInfor['TradeCount'];
    if ((stSiseStockInfor['TradeCount'] + nPlusCandleTradeCount) < (nMinusCandleTradeCount * 2)):
        return 0;

    if (bPrintLog > 0):
        PrintProgress(u"444");

    # 4. 이전 10일동안 7% 이상 급등한 종목 제외
    for nCandleIndex in range(nBaseBuyFastGrowDate):
        stCandelSiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nIndex - nCandleIndex - 1);
        if (stCandelSiseStockInfor == 0):
            continue;
        if ((stCandelSiseStockInfor['StartPrice'] == 0) or (stCandelSiseStockInfor['EndPrice'] == 0)):
            continue;

        nStartEndRate = (float(float(stCandelSiseStockInfor['EndPrice']) / float(stCandelSiseStockInfor['StartPrice'])) - 1) * 100;
        if (nStartEndRate >= nBaseBuyStartEndRate):
            return 0;

    if (bPrintLog > 0):
        PrintProgress(u"555");

    # 6. 매입 조건 형성 이후 5일 ~ 10일 사이에 가격을 만족할 경우, 매입!!
    nDateIndex = 0;
    nBuyDateIndex = 0;
    nBuyPrice = 0;
    bBuyCondition = 0;
    nBaseBuyPrice = (stSiseStockInfor['StartPrice'] + stSiseStockInfor['EndPrice']) / 2;
    for nDateIndex in range(nBaseBuyTradeDate):
        nBuyDateIndex = nIndex + nBaseBuyStartTradeDate + nDateIndex;
        stBuySiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nBuyDateIndex);
        if (stBuySiseStockInfor == 0):
            continue;

        if (nBaseBuyPrice > stBuySiseStockInfor['LowPrice']):
            nBuyPrice = nBaseBuyPrice;
            if (stBuySiseStockInfor['StartPrice'] < nBaseBuyPrice):
                nBuyPrice = stBuySiseStockInfor['StartPrice'];
            bBuyCondition = 1;
            break;
    # 거래 가격 만족 실패 ㅠ_ㅜ
    if (bBuyCondition == 0):
    ...
    nDateIndex = 0;
    nSellDateIndex = 0;
    nTargetSellPrice = nBuyPrice * (1 + (nTargetSellRate * 0.01));
    nDefenseSellPrice = nBuyPrice * (1 - (nDefenseSellRate * 0.01));
    nSellPrice = 0;
    bSellCondition = 0;
    nPrevPrice = nTargetSellPrice;
    nImpossibleRate = 30;
    for nDateIndex in range(nBaseSellTradeDate):
        nSellDateIndex = nBuyDateIndex + nDateIndex + 1;
        stSellSiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nSellDateIndex);
        if (stSellSiseStockInfor == 0):
            continue;
        if (stSiseStockInfor['TradeCount'] == 0):
            continue;
        if (nTargetSellPrice <= stSellSiseStockInfor['HighPrice']):
            nSellPrice = nTargetSellPrice;
            bSellCondition = 1;
            break;
        if (nDefenseSellPrice >= stSellSiseStockInfor['LowPrice']):
            nSellPrice = nDefenseSellPrice;
            bSellCondition = 1;
            break;
        if ((nPrevPrice > 0) and (stSellSiseStockInfor['EndPrice'] > 0)):
            nCurRate = float((float(stSellSiseStockInfor['EndPrice']) * 100) / float(nPrevPrice)) - 100;
            if (nCurRate > nImpossibleRate) or (nCurRate < (nImpossibleRate * -1)):
                return 0;
            nPrevPrice = stSellSiseStockInfor['EndPrice'];

    if (bPrintLog > 0):
        PrintProgress(u"777");

    # 12. 매도 가격 만족 실패할 경우, 40일 이후 시점에 손절매 ㅠ_ㅜ
    if (bSellCondition == 0):
        for nDateIndex in range(10):
            nSellDateIndex = nBuyDateIndex + nBaseSellTradeDate + nDateIndex + 1;
            stSellSiseStockInfor = CHART_GetSiseIndexInfor(astKospiInfor, astSiseStockInfor, nSellDateIndex);
            if (stSellSiseStockInfor == 0):
                continue;
            nSellPrice = stSellSiseStockInfor['StartPrice'];
            bSellCondition = 1;
    ...
    # 13. 40일 이후 매도 날짜를 찾지 못했을 경우, 현재 날짜로 판단
    if (bSellCondition == 0):
        nSellDateIndex = len(astKospiInfor) - 1;
        if (astSiseStockInfor.has_key(astKospiInfor[len(astKospiInfor) - 1]['Date'])):
            stSellSiseStockInfor = astSiseStockInfor[astKospiInfor[len(astKospiInfor) - 1]['Date']];
            nSellPrice = stSellSiseStockInfor['StartPrice'];
            bSellCondition = 1;
        else:
            # 현재 날짜의 매도 날짜를 찾지 못했을 경우, 상장 폐지로 판단
            nSellPrice = 0;

    if (bPrintLog > 0):
        PrintProgress(u"999");

    nEarningRate = ((float(nSellPrice) / float(nBuyPrice)) - 1) * 100;
    stSiseStockInfor['ChartFound'] = 1;

    if (bPrintLog > 0):
        PrintProgress(u"AAA");

    stChartPriceInfor = {};
    stChartPriceInfor['Type'] = stStockInfor['Type'];
    stChartPriceInfor['종목명'] = stStockInfor['Name'];
    stChartPriceInfor['날짜'] = astKospiInfor[nIndex]['Date'];
    stChartPriceInfor['시가'] = stSiseStockInfor['StartPrice'];
    stChartPriceInfor['종가'] = stSiseStockInfor['EndPrice'];
    stChartPriceInfor['거래량'] = stSiseStockInfor['TradeCount'];
    stChartPriceInfor['평균량'] = nAverageTradeCount;
    stChartPriceInfor['매수날짜'] = astKospiInfor[nBuyDateIndex]['Date'];
    stChartPriceInfor['매수가'] = nBuyPrice;
    stChartPriceInfor['매도날짜'] = astKospiInfor[nSellDateIndex]['Date'];
    stChartPriceInfor['매도가'] = nSellPrice;
    stChartPriceInfor['기간'] = nSellDateIndex - nBuyDateIndex;
    stChartPriceInfor['수익률'] = nEarningRate;
    astChartPriceInfor.append(0);
    astChartPriceInfor[len(astChartPriceInfor) - 1] = copy.deepcopy(stChartPriceInfor);

    if (bPrintLog > 0):
        PrintProgress(u"BBB");

    PrintProgress(u"[" + stChartPriceInfor['종목명'] + u"] " + astKospiInfor[nIndex]['Date'] + u" 시종가: [" + str(stSiseStockInfor['StartPrice']) + u" " + str(stSiseStockInfor['EndPrice']) + u"]", "yellow");
    PrintProgress(u"매수: [" + stChartPriceInfor['매수날짜'] + u" " + str(stChartPriceInfor['매수가']) + u"] 매도: [" + stChartPriceInfor['매도날짜'] + u" " + str(stChartPriceInfor['매도가']) + u" 기간: " + str(stChartPriceInfor['기간']) + u" 수익률: " + str(round(stChartPriceInfor['수익률'],2)), "green");
...
def EXCEL_WriteExcelFile(astKospiInfor, astKosdaqInfor, astStockInfor):
    ...
    if (gbChartSheet > 0):
        PrintProgress(u"[진행] 캔들 매매 출력");
        EXCEL_SetCandleTradeData();
        PrintProgress(u"[완료] 캔들 매매 출력");
...
def SISE_GetStockInfor(stStockInfor, stSiseInfor):   # IN (nStock: 종목코드), OUT (stSiseInfor: 시세 정보)
    nCodeUrl = 'http://www.etomato.com/home/itemAnalysis/ItemPrice.aspx?item_code=';
    nCodeUrl = nCodeUrl + stStockInfor['Code'];
    stResponse = GetUrlOpen(nCodeUrl);
    stPage = stResponse.read();
    stSoup = BeautifulSoup(stPage);
    tables = stSoup.findAll('table');
    ...
    for nIndex in range(3, nTableLen):
        ...
        stSiseInfor[stDate] = {};
        stSiseInfor[stDate] = int(astSplit[2].replace(",", ""));
    ...
    for nIndex in range(3, nTableLen):
        ...
        astSiseInfor[stDate] = {};
        astSiseInfor[stDate]['EndPrice'] = int(astSplit[2].replace(",", ""));
        astSiseInfor[stDate]['DiffPrice'] = int(astSplit[3].replace(",", ""));
        astSiseInfor[stDate]['StartPrice'] = int(astSplit[4].replace(",", ""));
        if ((astSiseInfor[stDate]['EndPrice'] > 0) and (astSiseInfor[stDate]['StartPrice'] > 0)):
            astSiseInfor[stDate]['StartEndRate'] = (float(float(astSiseInfor[stDate]['EndPrice']) / float(astSiseInfor[stDate]['StartPrice'])) - 1) * 100;
        else:
            astSiseInfor[stDate]['StartEndRate'] = 0;

        astSiseInfor[stDate]['HighPrice'] = int(astSplit[5].replace(",", ""));
        astSiseInfor[stDate]['LowPrice'] = int(astSplit[6].replace(",", ""));

        astSiseInfor[stDate]['TradeCount'] = int(astSplit[7].replace(",", ""));
        astSiseInfor[stDate]['TotalTradeCount'] = 0;
        astSiseInfor[stDate]['ChartCandidate'] = 0;
        astSiseInfor[stDate]['ChartFound'] = 0;
        astSiseInfor[stDate]['ChartTrade'] = 0;

        if (stDate == u"10-04-30"):
            break;

    global gastKospiInfor;
    global gastChartPriceInfor;

    CHART_SetTotalTradeCount(gastKospiInfor, astSiseInfor);

    nKospiLength = len(gastKospiInfor);
    for nKospiIndex in range(nKospiLength):
        CHART_CheckCandidate(stStockInfor, gastKospiInfor, astSiseInfor, nKospiIndex, gastChartPriceInfor);
...
def SISE_GetKospiInfor(astKospiInfor, astKosdaqInfor):
    ...
    for nIndex in range(nRetryCount):
        if (len(astKospiInfor) > 0):
            break;
        PrintProgress(str(nIndex + 1) + u"번째 시도");
        SISE_GetNonStockInfor('KOSPI', astKospiInfor);

    PrintProgress(u"[진행] KOSDAQ 정보 취합");
    for nIndex in range(nRetryCount):
        if (len(astKosdaqInfor) > 0):
            break;
        PrintProgress(str(nIndex + 1) + u"번째 시도");
...
def PrintProgress(stString, stColor = 0):
    astColorList = {
        'white':    "\033[39m",
        'yellow':   "\033[33m",
        'green':    "\033[32m",
    ...
    if (gbPrintProgress > 0):
        if (stColor == 0):
            print (astColorList['off'] + stString);
        else:
            print (astColorList[stColor] + stString);
...
def COMPANY_GetStockCode(astStockList): # OUT (gastChangeStockNameCodeList: 종목 이름 / 코드)
    ...
    for nStockType in range(len(astrStockType)):
        ...
        for nPageIndex in range(1, nMaxPage + 1):
            ...
            if (nStockCount >= gnMaxStockCount):
                break;
...
'''
bRet = COMPANY_GetStockFinanceInfor('KOSPI', u'한일진공', u'123840', gastStockInfor);
bRet = COMPANY_GetStockFinanceInfor('KOSDAQ', u'조아제약', u'034940', gastStockInfor);
bRet = COMPANY_GetStockFinanceInfor('KOSDAQ', u'현대공업', u'170030', gastStockInfor);
bRet = COMPANY_GetStockFinanceInfor('KOSDAQ', u'삼진', u'032750', gastStockInfor);
'''
